<template>
  <scroll>
    ...
    <a-row class="nav-left-header">
      <a-col :span="7">
        <b>{{$store.getters.queue.Workgroup}}人</b>
        <br>
        <span>{{$t('text.group.waiting')}}</span>
      </a-col>
      <a-col :span="2">
        <a-divider type="vertical" style="height:30px;margin-top:6px;"/>
      </a-col>
      <a-col :span="7">
        <b>{{$store.getters.queue.Agent}}人</b>
        <br>
        <span>{{$t('text.people.waiting')}}</span>
      </a-col>
      <a-col :span="8">
        <a-switch
          checkedChildren="接入"
      ...
      >
        ...
        <div class="ext">
          <div>
            ...
            <a-popconfirm title="确定关闭?" @confirm="closeSession(item.sessionId)">
