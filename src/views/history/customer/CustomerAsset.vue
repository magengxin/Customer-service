<template>
  <div class="customerAsset pb-3 border-t">
    <span class="case-title mb-2 font-weight-bold">{{$t('text.customer.HPMessage')}}</span>
    ...
    <scroll>
      ...
      >{{$t('text.asset.addProducts')}}
      </a-button>
    </scroll>

    <a-modal v-model="visible" :title="modelTitle" @ok="handleOk" :confirmLoading="confirmLoading" closable
             @cancel="closeModal" class="customer-asset">
...
<script>
  ...
  export default {
    ...
    created() {
      getCustomerProduct(this.customerInfo.id)
          .then(res => {
            if (res.code === 0) {
